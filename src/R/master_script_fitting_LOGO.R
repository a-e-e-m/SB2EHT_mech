...
# get IDs of groups with ID-SB data (T_bint_indices)
# nT_int equals the number of model runs for LOGO-CV
...
if (file.exists(file_models2run_LOGO_run)){
  ...
  models2run_LOGO_run <- tidyr::expand_grid(models2run[j,], LOGO_ID = T_bint_indices ) |>
...
if (cluster){
  ...
  for (k in seq(1, nT_int)){
    cmd <- paste0("Rscript ./src/R/fitting-job.R ", k, " ", file_models2run_LOGO_run)
    system(cmd)
  }
}
